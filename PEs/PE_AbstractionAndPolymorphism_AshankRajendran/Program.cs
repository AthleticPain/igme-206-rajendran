...
namespace Abstraction_Polymorphism_Demo
{
    class Program
    {
        ...
        private static void ShapesDemo()
        {
            ...
            do
            {
                choice = GetPromptedInput("What type of shape do you want " +
                    "(circle, semicircle, square, rectangle or Q to quit)?").ToUpper();
                switch (choice)
                {
                    // Get the radius for a new circle and add it to the list
                    case "CIRCLE":
                        double circleRadius = GetPromptedPositiveDouble("What is the radius?");
                        Circle newCircle = new Circle(circleRadius);
                    ...
                    case "SEMICIRCLE":
                        double semiCircleRadius = GetPromptedPositiveDouble("What is the radius?");
                        Shape newSemicircle = new Semicircle(semiCircleRadius);
                        shapes.Add(newSemicircle);
                        break;
        ...
        private static void PetsDemo()
        {
            ...
            for (int i = 0; i < pets.Length; i++)
            {
                ...
                else  // i%5 is equal to 4 in this case 
                {
                    pets[i] = new Pig(names[i], DateTime.Today);
                }
                pets[i].Speak();
            }
            Console.WriteLine();
